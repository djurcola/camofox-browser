...
export const decodoProvider = {
  ...
  buildProxyUrl(proxy, config) {
    ...
    return `${config.protocol || 'http'}://${user}:${pass}@${host}:${port}`;
  },
};
...
function proxyServer(protocol, host, port) {
...
function buildBackconnectProxy(config, provider, sessionId) {
  ...
  return {
    server: proxyServer(config.protocol, config.backconnectHost, config.backconnectPort),
    username,
...
export function createProxyPool(config) {
  ...
  function makeProxy(port) {
    return {
      server: proxyServer(config.protocol, host, port),
      username,
      password,
    };
  }
...
export function buildProxyUrl(pool, config) {
  ...
  if (pool.mode === 'backconnect') {
    ...
    const user = encodeURIComponent(proxy.username);
    const pass = encodeURIComponent(config.password);
    return `${config.protocol || 'http'}://${user}:${pass}@${config.backconnectHost}:${config.backconnectPort}`;
  }
  ...
  return `${config.protocol || 'http'}://${auth}${config.host}:${config.ports[0]}`;
}
