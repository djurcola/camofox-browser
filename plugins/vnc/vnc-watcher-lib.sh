...
display_for_xvfb_pid() {
  ...
  # Prefer the Unix socket actually opened by this Xvfb PID.  Docker can retain
  # stale X lock files from earlier container lifecycles whose numeric PID has
  # since been reused, so a lock alone is not sufficient evidence of ownership.
  ...
  # Fall back to a traditional X lock file when procfs socket ownership is not
  # available (for example, on a restricted non-Linux runtime).
